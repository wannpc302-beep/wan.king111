...
-- ปุ่ม WAN อยู่ด้านบนกลาง มีระยะห่างพอดี ไม่ชิดขอบบนเกินไป (35px)
local wanButton = Instance.new("TextButton")
wanButton.Name = "WANButton"
wanButton.Size = UDim2.new(0, 120, 0, 38)
wanButton.AnchorPoint = Vector2.new(0.5, 0)
wanButton.Position = UDim2.new(0.5, 0, 0, 35)
...
-- หน้าต่าง UI หลัก วางอยู่ตรงกลางหน้าจอพอดี (AnchorPoint 0.5, 0.5 & Position 0.5, 0.5)
...
-- ระบบกดสวิตช์แบบรองรับมือถือ: ใช้ MouseButton1Click เป็นตัวหลัก
-- ทั้งแถบสวิตช์ ข้อความ และ knob สามารถกดได้
local clickLock = {}

local function bindButton(button, callback)
    if not button then return end
    button.MouseButton1Click:Connect(function()
        if clickLock[button] then return end
        clickLock[button] = true
        callback()
        task.delay(0.12, function()
            clickLock[button] = nil
        end)
    end)
end

bindButton(switchBackground, toggleAntiLag)
bindButton(switchLabel, toggleAntiLag)
bindButton(switchKnob, toggleAntiLag)
bindButton(statusLabel, toggleAntiLag)

bindButton(fpsSwitch, function()
    setFPS(not fpsEnabled)
end)
bindButton(fpsLabelBtn, function()
    setFPS(not fpsEnabled)
end)
bindButton(fpsKnob, function()
    setFPS(not fpsEnabled)
end)

bindButton(noDarkSwitch, function()
    setNoDark(not noDarkEnabled)
    updateSimpleSwitch(noDarkSwitch,noDarkKnob,noDarkEnabled,true)
end)
bindButton(noDarkLabelBtn, function()
    setNoDark(not noDarkEnabled)
    updateSimpleSwitch(noDarkSwitch,noDarkKnob,noDarkEnabled,true)
end)
bindButton(noDarkKnob, function()
    setNoDark(not noDarkEnabled)
    updateSimpleSwitch(noDarkSwitch,noDarkKnob,noDarkEnabled,true)
end)
...
-- ลากแยกได้อย่างอิสระ ทั้งปุ่ม WAN, หน้าต่างหลัก, และ FPS Counter
makeDraggable(wanButton)
